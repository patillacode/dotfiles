...
tmux_session_picker() {
  ...
  defined=$(ls "$sessions_dir"/*.sh 2>/dev/null | grep -v '/_' | xargs -I{} basename {} .sh)
  all=$(printf "%s\n%s" "$running" "$defined" | sort -u | grep -v '^$' \
    | fzf --prompt="session > " --height=40%)
  [[ -z "$all" ]] && return
  # Script-backed sessions own their open logic (attach / resurrect-restore /
  # build) via tmux_smart_open in _lib.sh — never rebuild over a snapshot.
  if [[ -f "$sessions_dir/$all.sh" ]]; then
    bash "$sessions_dir/$all.sh"
  elif tmux has-session -t "=$all" 2>/dev/null; then
